...
void SystemTwo::initializeInteraction() {
	...
	for (const auto &r : states) {
          ...
          dipolemoment2 = coulombs_constant*cache.getElectricDipole(r.state.getFirstState(), c.state.getFirstState());     
            if(r.state.getM(0) - c.state.getM(0) == -1.){
                vec1[0] = (std::sqrt(1./2.)*(-dipolemoment1));
                vec1[1] = (std::sqrt(1./2.)*imagunit*dipolemoment1);
                vec1[2] = 0.;
            }
            if(r.state.getM(1) - c.state.getM(1) == -1.){
            ...
            if(r.state.getM(0) - c.state.getM(0) == 0.){
                vec1[0] = 0.;
                vec1[1] = 0.;
                vec1[2] = std::sqrt(1.)*dipolemoment1;
            }
            if(r.state.getM(1) - c.state.getM(1) == 0.){
                vec2[0] = 0.;
                vec2[1] = 0.;
                vec2[2] = std::sqrt(1.)*dipolemoment2;
            }
//             int dimension = system1.getNumStates() * system2.getNumStates();
            xxGTmatrix.reserve(5);
            if( r.idx==1 && c.idx==1 ){
                value = 0.01;
                this->addTriplet(xxGTmatrix,1,2,std::real(value));
            }
...
//                 this->addTriplet(xxGTmatrix,r.idx,c.idx,std::real(value));
//             }
//             if(vec1[1]*vec2[1]!=0.){
//                 value = vec1[1]*vec2[1];
...
void SystemTwo::addInteraction() {
    ...
    double angle_tolerance = 1e-14;
     if(GTbool){
      //Ganz umständlich?:
//       const int stateNum = system1.getNumStates() * system2.getNumStates();
      GreenTensor GT(x,zA,zB);
      eigen_sparse_t dummy;
